...
bool Crypto::Sign(BYTE *digest, BYTE *result)
{
  ...
  if (allZero == true)
  {
    Serial.println("Private key not set, generate a private key (eg use "
                   "Metamask) and call Contract::SetPrivateKey with it.");
  }
  else
  {
...
bool Crypto::Verify(const uint8_t *public_key, const uint8_t *message_hash,
...
string Crypto::ECRecoverFromHexMessage(string *signature, string *message)
{
  ...
  vector<uint8_t> messageBytes =
      Util::ConvertHexToVector((uint8_t *)(message->c_str()));
  // hash the full challenge
  Crypto::Keccak256((uint8_t *)messageBytes.data(), messageBytes.size(),
                    challengeHash);
...
string Crypto::ECRecoverFromHash(string *signature, BYTE *digest)
{
  ...
  Util::ConvertHexToBytes(signatureBytes, signature->c_str(),
                          ETHERS_SIGNATURE_LENGTH);

  // Perform ECRecover to get the public key - this extracts the public key of
  // the private key that was used to sign the message - that is, the private
  // key held by the wallet/metamask
  ...
  string recoveredAddress =
      Util::ConvertBytesToHex(recoverAddr, ETHERS_ADDRESS_LENGTH);
...
string Crypto::ECRecoverFromPersonalMessage(string *signature, string *message)
{
  ...
  Crypto::Keccak256((uint8_t *)challengeStr.c_str(), challengeStr.length(),
                    challengeHash);
  return ECRecoverFromHash(signature, challengeHash);
}