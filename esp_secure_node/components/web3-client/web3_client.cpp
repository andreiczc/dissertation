...
static constexpr auto *ETH_ADDRESS =
    "0xea8964911E02471f519300eE6D1eEc11236E3165";
...
std::string callContract(const std::string &contractAddress, std::string &data)
{
  ...
  string             wrappedOwnAddress(ETH_ADDRESS);
  uint32_t           nonce = web3.EthGetTransactionCount(&wrappedOwnAddress);

  ESP_LOGI(TAG, "Sending transaction. Nonce: %d", nonce);
