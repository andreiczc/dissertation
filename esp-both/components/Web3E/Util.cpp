...
uint32_t Util::RlpEncodeItem(uint8_t *output, const uint8_t *input,
                             uint32_t input_len)
{
...
string Util::ConvertBase(int from, int to, const char *s)
{
  ...
  {
    for (j = 0; j < ol; j++) // add the input digit times (base:to from^i) to
                             // the output cumulator
    {
      cums[j] += ts[j] * fs[i];
      int temp = cums[j];
      int rem  = 0;
 ...
 **/
vector<string> *Util::ConvertCharStrToVector32(const char     *resultPtr,
                                               size_t          resultSize,
...
string Util::ConvertWeiToEthString(uint256_t *weiVal, int decimals)
{
  ...
  // crawl along from the other side to find the first character or the last
  // zero
