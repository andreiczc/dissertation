  ...
  if (len > 0) {
    ...
    if (length > 0) {
      ...
      case 1:
        ...
        if (length == 8 && connectionValidCountdown == 0 &&
            memcmp(sessionBytes, packetBuffer + 2, 8) == 0) {
...
void UdpBridge::scanAPI(const BYTE *packet, APIReturn *apiReturn,
                        int payloadLength) {
  apiReturn->clear();
...
std::string UdpBridge::getArg(const BYTE *packet, int &index,
                              int payloadLength) {
...
boolean UdpBridge::isNewSession() {
  return (memcmp(sessionBytes, packetBuffer + 2, 8) != 0 &&
          memcmp(sessionBytes, packetBuffer + 10, 8) == 0);
}
